...
class FlaskController:
    ...
    def _register_routes(self):
        ...
        def convert_playlist():
            """
            Start batch conversion for a playlist.

            Currently supports:
            - Spotify playlists (open.spotify.com/playlist or spotify:playlist)
            """
            if not self.spotify_gateway:
                return jsonify({"error": "Spotify integration is not configured"}), 503
            ...
            try:
                ...
                source_url = str(data.get('youtube_url') or '').strip()
                if not source_url:
                    return jsonify({"error": "Missing required field: youtube_url"}), 400

                url_lower = source_url.lower()
                is_spotify_playlist = "open.spotify.com/playlist/" in url_lower or "spotify:playlist:" in url_lower
                if not is_spotify_playlist:
                    return jsonify({"error": "Only Spotify playlists are supported in this endpoint for now"}), 400

                audio_format = data.get('format', 'mp3')
                from utils.validators import validate_format, _to_bool
                is_valid_format, format_error = validate_format(audio_format)
                if not is_valid_format:
                    return jsonify({"error": format_error}), 400
                analyze_bpm_key = _to_bool(data.get('analyze_bpm_key', True))

                # Optional limit to avoid huge playlists
                try:
                    max_tracks = int(request.args.get('limit', '50'))
                except ValueError:
                    max_tracks = 50

                # Resolve playlist tracks from Spotify
                tracks = self.spotify_gateway.get_playlist_tracks(source_url, limit=max_tracks)
                if not tracks:
                    return jsonify({"error": "Playlist has no convertible tracks"}), 400

                task_entries = []

                def start_conversion_for_track(track_meta, youtube_url: str, task_id: str):
                    task = Task(
                        task_id=task_id,
                        youtube_url=youtube_url,
                        audio_format=audio_format,
                    ...
                    def run():
                        try:
                            ...
                            logger.error(f"Playlist track conversion failed: {e}")
                            self._update_task_progress(
                                task_id,
                                status='error',
                                progress=0,
                ...
                for t in tracks:
                    ...
                    try:
                        youtube_url, _ = self.convert_use_case.youtube_gateway.search_best_audio_video(query)
                    except Exception as e:
                        logger.warning(f"Skipping track {name!r} ({artists!r}) due to YouTube search error: {e}")
                        continue

                    task_id = self.task_gateway.create_task(youtube_url, audio_format, analyze_bpm_key)
                    start_conversion_for_track(t, youtube_url, task_id)
                ...
                if not task_entries:
                    return jsonify({"error": "Failed to start conversion for any track in the playlist"}), 400
                ...
                error_msg = str(e)
                logger.error(f"Error in /convert/playlist endpoint: {error_msg}")
                log_error_with_traceback(logger, e, "Error in /convert/playlist endpoint")
                return jsonify({"error": f"Internal server error: {error_msg}"}), 500
